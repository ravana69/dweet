...
for(h=540,x.fillRect(0,0,i=w=c.width|=0,w);i--;){for(x.beginPath(),j=4;j--;x.lineTo(960+X/(Z=(i/l|0)+2-t*4%1+C(p))*h,(5+S(Math.hypot(a=10-Z,X)-t*4+Math.atan2(a,X))/2)/Z*h))X=i%(l=70)-34+S(p=1.57*j);x.lineWidth=l/Z,x.strokeStyle='#40f8',x.stroke()}
  
//for(i=c.width|=l=70;i--;x.lineWidth=l/Z,x.stroke())for(x.beginPath(j=3);j--;x.lineTo(960+(X=i%l-34+S(p))/Z*700,(9+S(Math.hypot(9-Z,X)-t*9+Math.atan2(9-Z,X)))/Z*300-99))Z=C(p=1.57*j)+2+i/l-t*5%1
